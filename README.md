...
### 3.4. Đảm bảo Backend đang chạy & Cấu hình kết nối API
...
#### Cấu hình API URL cho App Mobile:
Mặc định, ứng dụng di động sẽ kết nối tới backend tại địa chỉ `http://10.0.2.2:8080/api/v1` (IP mặc định của Android Emulator để truy cập localhost của máy chủ).

Nếu bạn chạy trên các thiết bị hoặc môi trường khác, hãy tạo file `.env` từ `.env.example` trong thư mục `src/mobile-checkin/`:

```bash
cp src/mobile-checkin/.env.example src/mobile-checkin/.env
```

Và thiết lập giá trị `EXPO_PUBLIC_API_URL` tương ứng trong file `.env` vừa tạo:
*   **Android Emulator (Mặc định):** `http://10.0.2.2:8080/api/v1`
*   **iOS Simulator:** `http://localhost:8080/api/v1`
*   **Thiết bị di động thật (Physical Device):** `http://<IP_MÁY_TÍNH_CỦA_BẠN>:8080/api/v1` *(Lưu ý: Điện thoại và máy tính phải kết nối chung một mạng Wi-Fi).*
