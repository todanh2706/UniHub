...
2. **Backend API (Xử lý Bất đồng bộ):**
   -  đón nhận request (đã được bảo vệ bằng `@PreAuthorize("hasAnyRole('ADMIN', 'ORGANIZER')")`).
   -  thực hiện các bước:
...
2. **Backend API (Pipeline xử lý):**
   -  đón nhận request và thực hiện chuỗi xử lý Pipeline:
     ...
     - **Stage 3 (Trích xuất văn bản):**  thực hiện đọc tệp PDF và trích xuất toàn bộ văn bản thô, lưu kết quả thô vào DB và cập nhật trạng thái `"EXTRACTED"`.
     - **Stage 4 (Tóm tắt bằng AI):** Gọi  truyền văn bản thô. Dịch vụ gửi yêu cầu HTTP POST đến cổng OpenRouter API để lấy tóm tắt ngắn gọn 3-5 câu bằng tiếng Việt từ mô hình `openai/gpt-4o-mini`.
...
- **Phân quyền chặt chẽ (RBAC Guard):**
  - Toàn bộ các endpoint tại  và  bắt buộc phải có chú thích bảo vệ `@PreAuthorize("hasAnyRole('ADMIN', 'ORGANIZER')")`. Các vai trò không thuộc nhóm này (như `STUDENT` hoặc chưa xác thực) hoàn toàn không có quyền gọi hoặc tương tác với hệ thống.
