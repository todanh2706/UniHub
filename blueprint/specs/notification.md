...
2. **Backend API (Dispatch & Strategy):**
   - [NotificationService.sendRegistrationConfirmation()](file:///Users/todanh/Downloads/UniHub/src/backend/src/main/java/vn/unihub/backend/service/NotificationService.java) được gọi và nhận thông tin thực thể `Registration`.
   - Spring Boot tự động tiêm danh sách toàn bộ các Bean thực thi interface [NotificationChannel](file:///Users/todanh/Downloads/UniHub/src/backend/src/main/java/vn/unihub/backend/service/notification/NotificationChannel.java). Hệ thống lặp qua từng kênh để gửi:
     - **Kênh In-App ([InAppNotificationChannel](file:///Users/todanh/Downloads/UniHub/src/backend/src/main/java/vn/unihub/backend/service/notification/InAppNotificationChannel.java)):** Tạo trực tiếp một thực thể `Notification` chứa tiêu đề, nội dung và liên kết với tài khoản người dùng, sau đó lưu ngay vào PostgreSQL thông qua `NotificationRepository`.
     - **Kênh Email ([EmailNotificationChannel](file:///Users/todanh/Downloads/UniHub/src/backend/src/main/java/vn/unihub/backend/service/notification/EmailNotificationChannel.java)):** Đóng gói thông tin (email sinh viên, tên, tên workshop, thời gian bắt đầu, phòng học) vào đối tượng JSON `RegistrationConfirmationPayload`. Sinh một thực thể `OutboxEvent` có kiểu sự kiện `"REGISTRATION_CONFIRMED"`, trạng thái `"PENDING"`, và lưu trực tiếp vào bảng `outbox_events`.
...
3. **Tiến trình Xử lý nền (Outbox Worker & SMTP):**
   - [OutboxWorker.java](file:///Users/todanh/Downloads/UniHub/src/backend/src/main/java/vn/unihub/backend/worker/OutboxWorker.java) kích hoạt định kỳ 5 giây một lần (cấu hình qua `@Scheduled`).
   ...
   - Duyệt qua danh sách các handler để tìm handler hỗ trợ. [EmailRegistrationConfirmationOutboxHandler](file:///Users/todanh/Downloads/UniHub/src/backend/src/main/java/vn/unihub/backend/worker/handler/EmailRegistrationConfirmationOutboxHandler.java) bắt được sự kiện `"REGISTRATION_CONFIRMED"`:
...
4. **Phản hồi & Hiển thị trên Client:**
   ...
   - **In-App:** Component [NotificationBell.tsx](file:///Users/todanh/Downloads/UniHub/src/frontend/src/components/NotificationBell.tsx) trên thanh điều hướng thực hiện polling API `/notifications/unread-count` định kỳ 30 giây qua hook [useNotifications.ts](file:///Users/todanh/Downloads/UniHub/src/frontend/src/hooks/useNotifications.ts). Khi phát hiện có thông báo mới, biểu tượng quả chuông hiển thị huy hiệu (Badge) số lượng chưa đọc màu đỏ nổi bật. Người dùng click mở chuông sẽ xem được tiêu đề và nội dung mô tả chi tiết với hiệu ứng mượt mà điều phối bởi `framer-motion`.
...
Thêm các biến môi trường cấu hình Token và ID nhóm/kênh Telegram vào file [application.yaml](file:///Users/todanh/Downloads/UniHub/src/backend/src/main/resources/application.yaml):
```yaml
application:
  security:
    telegram:
...
Tạo tệp tin mới implement interface [NotificationChannel](file:///Users/todanh/Downloads/UniHub/src/backend/src/main/java/vn/unihub/backend/service/notification/NotificationChannel.java):
```java
...
- [ ] AC5: Đảm bảo tính mở rộng cao (OCP), cho phép tích hợp kênh Telegram mới chỉ bằng cách thêm lớp Component mà không được phép thay đổi tệp tin [NotificationService.java](file:///Users/todanh/Downloads/UniHub/src/backend/src/main/java/vn/unihub/backend/service/NotificationService.java).
