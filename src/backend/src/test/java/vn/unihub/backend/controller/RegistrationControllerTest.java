...
class RegistrationControllerTest {
        ...
        void setUp() {
                ...
                entityManager.persist(UserRole.builder().id(new UserRoleId(studentUser2.getId(), studentRole.getId()))
                                .user(studentUser2).role(studentRole).build());
                entityManager.persist(
                                UserRole.builder().id(new UserRoleId(organizerUser.getId(), organizerRole.getId()))
        ...
        void organizerSummary_success() throws Exception {
                mockMvc.perform(get("/api/v1/organizer/workshops/{workshopId}/registrations/summary",
                                freeWorkshop.getId())
                                .with(httpBasic("organizer@unihub.local", "secret")))
                                .andExpect(status().isOk())
